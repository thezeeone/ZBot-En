import { Formatters, EmbedBuilder, ApplicationCommandOptionType, ChatInputCommandInteraction } from "discord.js"
...
const gtwCommand: Cmd = {
    ...
    async execute(interaction: ChatInputCommandInteraction<"cached">): Promise<any> {
        const wordOrSentence = (<ChatInputCommandInteraction<"cached">>interaction).options.getString('word-or-sentence', true).replace(/\s+/, () => ' ')

        if (wordOrSentence.length < 3) return await interaction.reply({
            content: `String must be at least 3 characters long. String length is ${Formatters.bold(wordOrSentence.length.toString())}.`,
            ephemeral: true
        })
        ...
        .setDescription(`${Formatters.bold('Word or Sentence')} ${Formatters.inlineCode(layOut.map(s => s.join(' ')).join('  '))}\n\n${Formatters.italic('Can you guess this one...? Simply type in either a letter in chat to see if it\'s included, or if you can, try and guess the entire word/statement and gain experience points!\nCase-sensitive')}`)
        ...
        .addFields([
            ...
            { name: 'Correct Characters', value: correctCharacters.length === 0 ? 'none' : correctCharacters.map(s => Formatters.inlineCode(s)).join(' '), inline: true },
            { name: 'Incorrect Characters', value: incorrectCharacters.length === 0 ? 'none' : incorrectCharacters.map(s => Formatters.inlineCode(s)).join(' '), inline: true }
        ])
            ...
            if (msg.content.length === 1) {
                ...
                if (correctCharacters.includes(letter)) {
                    ...
                    if (layOut.flat().flat().every(s => s !== '_')) {
                        ...
                        .setDescription(`${Formatters.bold('The word/sentence has been guessed! It was...')}\n${Formatters.inlineCode(wordOrSentence)}\n\nCongratulations ${msg.author}, you won!`)
                        .setFields([])
                        game.edit({ embeds: [embed] })
                        await msg.reply(`Congratulations, you guessed it! Click here to jump to original game: ${game.url}\nYou will gain **${Formatters.inlineCode(String(numGuesses * 5))} experience points**.`)
                    ...
                    embed
                    .setDescription(`${Formatters.bold('Word or Sentence')} ${Formatters.inlineCode(layOut.map(s => s.join(' ')).join('  '))}\n\n${Formatters.italic('Can you guess this one...? Simply type in either a letter in chat to see if it\'s included, or if you can, try and guess the entire word/statement!\nCase-sensitive')}`)
                    .setFields([
                        { name: 'Guesses Remaining', value: String(numGuesses), inline: true },
                        { name: 'Correct Characters', value: correctCharacters.length === 0 ? 'none' : correctCharacters.map(s => Formatters.inlineCode(s)).join(' '), inline: true },
                        { name: 'Incorrect Characters', value: incorrectCharacters.length === 0 ? 'none' : incorrectCharacters.map(s => Formatters.inlineCode(s)).join(' '), inline: true }
