import { ActionRowBuilder, ApplicationCommandType, bold, ButtonBuilder, ButtonStyle, ChannelType, ComponentType, DMChannel, EmbedBuilder, Guild, italic, MessageContextMenuCommandInteraction, ModalBuilder, OverwriteType, PermissionsBitField, SelectMenuBuilder, TextChannel, TextInputBuilder, TextInputStyle, time } from "discord.js"
import { repliedMessages } from ".."
import { TicketSystemModel, TicketTypes } from "../database"
import { commaList, pluralise } from "../util"
import { Cmd } from "./command-exports"

const reportMessageCommand: Cmd = {
    data: {
        name: 'report-message',
            ...
            .then(async (DMMessage) => {
                ...
                const reasons = [
                    ...
                    },
                    {
                        label: 'Spam',
                        value: 'spam',
                        description: 'Repetitive messages, attachment spam, etc.'
                    },
                    {
                    ...
                    },
                    {
                        label: 'NSFW',
                        value: 'nsfw',
                        description: 'Content deemed unsafe for viewing'
                    },
                    {
                        label: 'Threats',
                        value: 'threat',
                ...
                await DMMessage.edit({
                    content: `Please select a reason, or multiple, for reporting this message. ${italic(`A response is required ${time(Math.floor(Date.now() / 1000) + 180)}`)}`,
                    embeds: [
                        new EmbedBuilder()
                            .setAuthor({
                            ...
                            .setFields([
                                {
                                    name: 'Attachments',
                                    value: [...message.attachments.values()].length
                                        ? `${pluralise(message.attachments.size, 'attachment')}\n${[...message.attachments.values()].map(a => `[${a.name || 'unknown'}${a.contentType ? `\.${a.contentType}` : ''}](${a.proxyURL})`).join('\n')}`
                                        : 'None'
                                },
                                {
                                    name: 'Created',
                                    value: `${time(Math.floor(message.createdTimestamp / 1000), 'F')} (${time(Math.floor(message.createdTimestamp / 1000), 'R')})`
                                }
                            ])
                    ],
                    components: [
                                ...
                                embeds: [
                                    new EmbedBuilder()
                                        ...
                                        .addFields([
                                            ...
                                            {
                                                name: 'Message Author',
                                                value: `[${message.author.tag}](${message.author.displayAvatarURL({ forceStatic: false })}) (${message.author.id})`
                                            },
                                            {
                                                name: 'Message Content',
                                                value: message.content || italic('None')
                                            },
                                            {
